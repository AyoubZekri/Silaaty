...
public function syncData(Request $request, $table)
{
    ...
    foreach (array_chunk($payload, $batchSize) as $batch){
      foreach ($batch as $data) {
        ...
        $localUpdatedAt = $localUpdatedAt->addMinutes(30);    
        ...
        if (!$existing) {
            ...
            $data['updated_at'] = isset($data['updated_at'])
                ? Carbon::parse($data['updated_at'])->addMinutes(30)->format('Y-m-d H:i:s')
            ...
            if ($localUpdatedAt->gt($serverUpdatedAt)) {
                $data['updated_at'] = now()->addMinutes(30)->format('Y-m-d H:i:s');
                try {
